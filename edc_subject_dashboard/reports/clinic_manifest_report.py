...
class ClinicManifestReport(Report):

    def __init__(self, requisitions=None, user=None, **kwargs):
        super().__init__(**kwargs)
        self.requisitions = requisitions
        self.contact_name = f'{user.first_name} {user.last_name}'
    ...
    def description(self):
        specimen_count = sum([obj.item_count for obj in self.requisitions])
        specimen_types = list(
            set([obj.panel_object.aliquot_type.name for obj in self.requisitions]))
        description = {
            'box_count': 1,
        ...
        box_word = 'box'
        specimen_word = 'specimen' if specimen_count == 1 else 'specimens'
